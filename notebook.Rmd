...
> Future: Try multiple classifications models: Decision Tree, Random Forest, SVM, Gradient Boosting Machine
...
```{r}
matriz_costos = matrix(c(0, 1, 5, 0), 2)
colnames(matriz_costos) = rownames(matriz_costos) = getTaskClassLevels(task)
```
```{r}
...
```{r}
lrn.multinom= makeLearner("classif.multinom", predict.type = "prob", trace = FALSE)
mod.multinom = train(lrn.multinom, task)
pred.multinom = predict(mod.multinom, task = task)
```

Calculate the theoretical threshold for the positive class
```{r}
th = matriz_costos[2,1]/(matriz_costos[2,1] + matriz_costos[1,2])
th
```
```{r}
pred.th = setThreshold(pred.multinom, th)
```
...
## Tuning threshold
```{r}
lrn.gbm = makeLearner("classif.gbm", predict.type = "prob", distribution = "bernoulli")
ps = makeParamSet(
  makeIntegerParam("interaction.depth", lower = 1, upper = 5)
)
ctrl.tune = makeTuneControlRandom(maxit = 2, tune.threshold = TRUE)
lrn.gbm = makeTuneWrapper(lrn.gbm, par.set = ps,control = ctrl.tune, resampling = cv2)
r = resample(lrn.gbm, task, cv3, extract = getTuneResult)
```
```{r}
print(r$extract)
```
